#!/usr/bin/env bash
# Ops 一键部署：逻辑在 aws-ops/code/deploy_orchestrator.py（子命令 aws）。
...
#
# 运维脚本在项目根下 aws-ops/：例如 aws-ops/database/pg_ops.sh、aws-ops/lib/read_deploy_config.py、
# aws-ops/aws_ops.sh；与部署配置、SSH 约定一致。
#
# 等价：python3 aws-ops/code/deploy_orchestrator.py aws [选项...]
