...
``build_arg_parser()`` 等。统一入口：``./aws-ops/database/pg_ops.sh``。
...
本地连接（与 aws-ops/database/test_local_postgres.py 一致）：
  DATABASE_URL → baasapi/database_config.json → example → 默认 127.0.0.1/hztech

远端：HZTECH_REMOTE_PG_*、POSTGRES_PASSWORD
SSH：aws-ops/lib/read_deploy_config.py --role baasapi；HZTECH_SSH_PG_TARGET、HZTECH_SSH_KEY_FILE、HZTECH_SSH_OPTS

导入前默认在远端 sudo postgres 建库/角色/schema；跳过：--no-ensure-remote-db

运维菜单（网络/SSH/HTTP、本机与 AWS PG 自检）：aws-ops/aws_test.sh

用法：
  ./aws-ops/database/pg_ops.sh
  ./aws-ops/database/pg_ops.sh import --dry-run
  python3 aws-ops/database/pg_ows_import.py
  python3 aws-ops/database/pg_ows_import.py --dry-run
  python3 aws-ops/database/pg_ows_import.py --backup-remote-first
  python3 aws-ops/database/pg_ows_import.py --from-download
  python3 aws-ops/database/pg_ows_import.py --from-flutterapp-pg
  python3 aws-ops/database/pg_ows_import.py --from-sql /path/to/dump.sql
  python3 aws-ops/database/pg_ows_import.py --dump-to .temp-cursor/local.sql
  python3 aws-ops/database/pg_ows_import.py --dump-app-schema-only --dump-to .temp-cursor/app.sql
  python3 aws-ops/database/pg_ows_import.py --remote-db hztechapp
"""
...
AWS_OPS_ROOT = Path(__file__).resolve().parents[1]
...
def load_deploy_json(role: str) -> dict:
    cmd = [
        sys.executable,
        str(AWS_OPS_ROOT / "lib" / "read_deploy_config.py"),
        "--json",
        "--role",
        role,
    ]
...
def build_arg_parser(default_remote_db: str = "hztech") -> argparse.ArgumentParser:
    ...
    ap.add_argument(
        ...
        "（与 aws-ops/database/pg_dump_to_aws_alpha.sh 的 HZTECH_PG_DUMP_APP_ONLY 一致）。"
...
def main(argv: list[str] | None = None) -> int:
    ...
    print(
        "验证可执行: HZTECH_REMOTE_PG_DB=%s ./aws-ops/database/pg_ops.sh test-aws" % rdb,
    )
    return 0
