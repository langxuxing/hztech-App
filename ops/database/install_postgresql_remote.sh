...
# 将本机 PostgreSQL 备份导入此机：aws-ops/database/pg_ops.sh import 或 pg_ows_import.py（远端仅监听 127.0.0.1，经 SSH 管道导入）。
# 导入后验证：aws-ops/database/pg_ops.sh test-aws，或 aws-ops/aws_test.sh 菜单选 7。
# 支持 Amazon Linux 2023 / Amazon Linux 2（yum/dnf）与 Debian/Ubuntu（apt）。
#
# 用法：
#   bash aws-ops/database/install_postgresql_remote.sh [ssh_target]
#   HZTECH_PG_USER=hztech HZTECH_PG_PASSWORD='...' HZTECH_PG_DB=hztech bash aws-ops/database/install_postgresql_remote.sh aws-alpha
#
# 可选：
