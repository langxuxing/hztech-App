...
数据库自检脚本。亦可通过：./aws-ops/database/pg_ops.sh test-aws 或 aws-ops/aws_test.sh 选 7

远端 Postgres 监听 127.0.0.1，经 SSH 在 BaasAPI 机上执行 psql（较旧版 shell 快检信息更全）。

SSH / 部署：
  默认读取 baasapi/deploy-aws.json（与 aws-ops/lib/read_deploy_config.py --role baasapi 一致）。
...
用法：
  ./aws-ops/database/pg_ops.sh test-aws
  python3 aws-ops/database/test_aws_postgres.py
  python3 aws-ops/database/test_aws_postgres.py --role baasapi
  HZTECH_REMOTE_PG_PASSWORD='...' python3 aws-ops/database/test_aws_postgres.py
"""
