...
# 远端库仅监听 127.0.0.1:5432（见 aws-ops/database/install_postgresql_remote.sh），故通过 SSH 管道执行 psql。
#
# 统一入口：./aws-ops/database/pg_ops.sh dump-alpha（本文件为纯 bash 实现，功能与 pg_ows_import 重叠时可任选其一）。
# 依赖：本机已安装 pg_dump；ssh 可登录目标机；远端已创建目标库与用户。
# 数据迁移推荐：./aws-ops/database/pg_ops.sh 或 python3 aws-ops/database/pg_ows_import.py
#
# 用法：
#   ./aws-ops/database/pg_dump_to_aws_alpha.sh
#   ./aws-ops/database/pg_dump_to_aws_alpha.sh --dry-run
#   ./aws-ops/database/pg_dump_to_aws_alpha.sh --backup-remote-first
#
...
#   HZTECH_PG_DUMP_APP_ONLY=1 ./aws-ops/database/pg_dump_to_aws_alpha.sh
...
#   HZTECH_PG_DUMP_SCHEMAS="public flutterapp" ./aws-ops/database/pg_dump_to_aws_alpha.sh
#
...
#
# 导入后验证（远端 psql）：可用 pg_ops.sh test-aws 或 test_aws_postgres.py
#
set -euo pipefail
...
eval "$("$SCRIPT_DIR/../lib/read_deploy_config.py" --bash-export --role baasapi)"
