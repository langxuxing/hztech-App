# -*- coding: utf-8 -*-
"""
模拟 Flutter App 登录与交易所账户相关请求链：登录 → account-profit → accounts →
...
class TestAppAccountFlowLikeFlutter:
    ...
    def test_login_then_account_apis_match_app_contract(self, client, auth_headers):
        ...
        if bot_id is None:
            pytest.skip("当前 accounts 列表中无 Account_List 的 OKX_* 账户，跳过 per-account 接口")

        # 3) profit-history（收益曲线）
        r = client.get(
            f"/api/accounts/{bot_id}/profit-history?limit=50",
        ...
        r = client.get(
            f"/api/accounts/{bot_id}/pending-orders",
            headers=auth_headers,
        )
        assert r.status_code == 200
        ord_data = r.get_json()
        ...
        # 7) seasons
        r = client.get(
            f"/api/accounts/{bot_id}/seasons?limit=10",
            headers=auth_headers,
        )
        assert r.status_code == 200
        sea = r.get_json()
    ...
    def test_account_profit_accounts_align_with_tradingbots_ids(
        ...
        """盈亏列表中的 bot_id 应能在 GET /api/accounts 列表中找到（或来自同一后端数据源）。"""
        ...
        for a in accounts:
            ...
            if bid:
                assert bid in bot_ids, (
                    f"account-profit 中的 bot_id={bid} 应在 /api/accounts 列表中"
                )
