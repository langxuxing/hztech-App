...
/// 账户收益详情页核心实现（数据、下拉切换、资产/曲线/月度/持仓/赛季）。
/// 由 [AccountProfitScreen]（移动）与 [WebAccountProfitScreen]（Web）分别包装入口。
class AccountProfitPage extends StatefulWidget {
  const AccountProfitPage({
    super.key,
  ...
  /// true：Web 端标题/返回与宽屏双栏（≥960px）；false：移动端单列与「账户收益」标题
  ...
  @override
  State<AccountProfitPage> createState() => _AccountProfitPageState();
}

class _AccountProfitPageState extends State<AccountProfitPage> {
  ...
  @override
  void didUpdateWidget(covariant AccountProfitPage oldWidget) {
  ...
  Widget _buildBodyScrollable() {
    final wide =
        widget.useWebLayout && MediaQuery.sizeOf(context).width >= 960;
    if (wide) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: widget.embedInShell
          ? null
          : AppBar(
              leading: widget.useWebLayout && canPop
                  ? IconButton(
                      icon: const Icon(Icons.arrow_back),
                      onPressed: () => Navigator.of(context).pop(),
                    )
                  : null,
              automaticallyImplyLeading: !(widget.useWebLayout && canPop),
              title: Text(
                widget.useWebLayout ? '账户详情' : '账户收益',
      ...
      Column(
        ...
        children: [
          ...
          if (_snapshots.isNotEmpty) ...[
            ...
            SizedBox(
              height: widget.useWebLayout ? 240 : 192,
