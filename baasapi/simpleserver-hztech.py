#!/usr/bin/env python3
"""hztech bot：每分钟查询 OKX BTC 永续合约价格并打印。启动/退出时写入数据库 strategy_events；支持 SIGTERM 优雅退出。"""
...
# 与 app 共用同一 DB（baasapi/sqlite/tradingbots.db），需在 baasapi 目录或项目根下以 python baasapi/simpleserver-hztech.py 运行
import db as _db

OKX_TICKER_URL = "https://www.okx.com/api/v5/market/ticker?instId=BTC-USDT-SWAP"
INTERVAL_SEC = 60
BOT_ID = "simpleserver-hztech"
...
def fetch_btc_price() -> str | None:
    ...
    try:
        req = Request(OKX_TICKER_URL, headers={"User-Agent": "hztech-simpleserver-hztech/1.0"})
