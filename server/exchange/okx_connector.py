...
try:
    ...
    logging.info("✅ 成功导入 QTrader 核心模块（使用统一Exchange接口）")
...
class ExchangeManager:
    ...
    def __init__(self):
        ...
        if HAS_EXCHANGE_MANAGER:
            ...
            logger.info("使用统一的ExchangeManager")
        else:
            ...
            logger.info(f"交易所管理器初始化完成（旧实现），已连接 {len(self.exchanges)} 个交易所")
        ...
        for exchange_type in enabled_exchanges:
            try:
                ...
                if exchange_type == ModelExchangeType.OKX and not HAS_EXCHANGE_MANAGER:
                    ...
                    logger.info("OKX 使用本地 okx 模块（余额/持仓/行情）")
                    continue
                ...
                if exchange:
                    ...
                    self.exchange_configs[exchange_type] = config
                    logger.info(f"{exchange_type.value} 交易所连接成功")
                else:
    ...
    async def close_all_connections(self) -> None:
        ...
        for exchange_type, exchange in self.exchanges.items():
            if exchange is _OKX_NATIVE_SENTINEL:
                logger.info(f"{exchange_type.value} 使用本地模块，无需关闭连接")
                continue
            try:
                if exchange and hasattr(exchange, 'close'):
                    await exchange.close()
                logger.info(f"{exchange_type.value} 交易所连接已关闭")
            except Exception as e:
