...
/// 权益曲线：用快照的 `equityUsdt` 画线（仪表盘卡片与权益走势一致）。
class EquityUsdtLineChart extends StatelessWidget {
  const EquityUsdtLineChart({super.key, required this.snapshots});
  ...
  Widget build(BuildContext context) {
    if (snapshots.isEmpty) return const SizedBox.shrink();
    final spots = <FlSpot>[];
    final firstY = snapshots.first.equityUsdt;
    var minY = firstY;
    var maxY = firstY;
    for (var i = 0; i < snapshots.length; i++) {
      final b = snapshots[i].equityUsdt;
      spots.add(FlSpot(i.toDouble(), b));
      if (b < minY) minY = b;
      if (b > maxY) maxY = b;
    }
        ...
        lineTouchData: LineTouchData(
          ...
          touchTooltipData: LineTouchTooltipData(
            getTooltipItems: (touchedSpots) {
              return touchedSpots.map((spot) {
                return LineTooltipItem(
                  '${spot.y.toStringAsFixed(2)} USDT（权益）',
                  TextStyle(
                    color: lineColor,
                    fontWeight: FontWeight.w600,
                    fontSize: 12,
